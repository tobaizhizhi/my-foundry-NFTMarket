...
contract NFTMarketplaceCore is ReentrancyGuard,NFTMarketplaceData{
    ...
    function listNFT(address nftContract,uint256 tokenId,uint256 priceWei,uint256 expireDuration) external {
        if(nftContract==address(0)) revert NFTMarketplace__InvalidNFTContract();
        if(ERC721(nftContract).ownerOf(tokenId)!=msg.sender) revert NFTMarketplace__NFTNotOwnedBySender();
    ...
    function cancelListing(address nftContract,uint256 tokenId) external{
        ...
        if(listing.seller!=msg.sender) revert NFTMarketplace__NotListingSeller();
    ...
    function buyNFT(address nftContract,uint256 tokenId) external payable nonReentrant{
        ...
        if(listing.expireTime < block.timestamp) revert NFTMarketplace__ListingExpired();
        if(!listing.isActive) revert NFTMarketplace__ListingNotActive();
        if(msg.value < priceWei) revert NFTMarketplace__MoneyIsNotEnoughToBuy();
        if(ERC721(nftContract).ownerOf(tokenId)!= seller) revert NFTMarketplace__SellerNotNFTOwner();
